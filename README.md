...
##### Метрика
Оценка качества будет замеряться по метрике hitrate@5.
...
### Анализ данных
При EDA единственным препятствием стал размер датасета логов пользователей - 77 миллионов строк.
...
_2. Вторая модель представляет собой тот же CatboostClassifier с обучением на полученных с помощью DL эмбеддингами
текстов постов. Эмбеддинги текстов получены с помощью эмбеддингов слов, в свою очередь
полученные обучением Word2Vec и дальнейшими преобразованиями._
